...
function HideUpdate
{
	PARAM([string]$UpdateTitle, [bool]$Hide)
	# bool Hide = 0 means unHide update
	# bool Hide = 1 means hide Update
	
	foreach($update in $Global:count)
	{
		if ($update.Title -eq $UpdateTitle)
		{
			$update.IsHidden = $Hide
			return  $Global:RootCauseDetected = $false
		}
	}
}
#End of the function
...
If($Global:SelectedUpdate -ne "")
{
   $Hideitems = ""
   ForEach ($item in $Global:SelectedUpdate.Split(";"))
   {
        $temp = HideUpdate $item 1
		#Pop-Msg "returned value : $temp"
		#Pop-Msg "here 2"
		ForEach($i in $Global:hideitemNames)
		{
			if($item -eq $i.Name)
			{
				$Hideitems += $i.Name + "`n"
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUFyxsvefspyMKaOHv84wp3X+d
# ABKgghWCMIIEwzCCA6ugAwIBAgITMwAAAHD0GL8jIfxQnQAAAAAAcDANBgkqhkiG
...
# HAYKKwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFLAQ
# BCyR4FgTLhlt1Fnq4vT8AqOuMGYGCisGAQQBgjcCAQwxWDBWoDyAOgBXAHUAUwBo
# AG8AdwBIAGkAZABlAF8AUgBTAF8ASABpAGQAZQBVAHAAZABhAHQAZQBzAC4AcABz
# ADGhFoAUaHR0cDovL21pY3Jvc29mdC5jb20wDQYJKoZIhvcNAQEBBQAEggEAFqq/
# yR/9KTjyWFR8NNQ7UJJuo5lgXSphv8Vl4mf5feUcQejy6L3c9wZ8rDzmM+8w22vN
# qwElM0L/FAvQQfjFm/AWDK6T+ZSgZ2i0HcQt1CcGwV33xPa2EKcsq2sswIJzL6rr
# AuYvAb7E0ws8fU0UemjyE95CeyLtZNv0biVvpGtZi8v+75FnK+LC41llqysjLdbp
# kcfW29w3BnDx0RVNDxK4yMou+V28Lm+N8j0ctT1T46DafUti+yzx+4PECM4pI+Iu
# l63VPpe7RuoGjzForf9BsdTklfCzLohkefvvyH4hL4S/oWcln/Z8Ti7aDfY+Vpt+
# P1ioZoUOPhRQHMQKS6GCAigwggIkBgkqhkiG9w0BCQYxggIVMIICEQIBATCBjjB3
...
# MQ8XDTE1MDYxNTIwMDY1N1owIwYJKoZIhvcNAQkEMRYEFNm5D2mlGvBbCqMSzogv
# oCqGYikMMA0GCSqGSIb3DQEBBQUABIIBABKJJl7bukeuiuc37vO6LV3/nY1LpACk
# kFJj8aV4fFCHpDXxraJztuK5cRD2MdLu8Uec4XOvZXLiIQSoaG7tveU/MK73+oqJ
# J0g/eEOuiwCE8vq9nZC55CrL/rf/+FYQPj6n73BFmHl5XLAdt7cJl87X9mj4h8JJ
# RAAUtCDOya9Bggc5xZkSY9MrBh7JYQ6ZuHjP/+0gAK7vpL2FI6R7jPYnZyAxeXui
# mKHNkHXNUYR/PPL5bDQN7e9E95C2DGae0+1pliPorNKd1AtfrVMGpNq3q0c66JU5
# PbV1y4ETdw86v6ZmhsQidTU5EP/bvPSZbsMPpTRYjoWCb4tOhifiSY0=
