...
Import-LocalizedData -BindingVariable string_RCUnHide -FileName RC_UnHideUpdates

$Global:RootCauseId = "RC_UnHideUpdates"
$Global:RootCauseDetected = $true

ForEach ($i In $Global:Updatelist)
{
	if($i.IsHidden -eq $true)
	{
		$Global:unhideitemNames += @{"Name" = ($i.Title); "Value" = ($i.Title); "Description" = ""; "ExtensionPoint" = ""}
	}
}
if($Global:unhideitemNames -ne $null)
{
	$Global:items = Get-DiagInput -Id "UnHideChoice" -Choice $Global:unhideitemNames
...
{
	$no_update = $string_RCUnHide.ID_RC_UNHIDE
	Get-DiagInput -id IT_NoUpdate -Parameter @{"Noupdate" = "$no_update"}
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUuTp3rXlu2nGVt/Nms5yipJeb
# iUOgghWCMIIEwzCCA6ugAwIBAgITMwAAAHD0GL8jIfxQnQAAAAAAcDANBgkqhkiG
...
# HAYKKwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFBUK
# rN3sHgw5zEG+eTb6gwedvA+iMGoGCisGAQQBgjcCAQwxXDBaoECAPgBXAHUAUwBo
# AG8AdwBIAGkAZABlAF8AUgBDAF8AVQBuAEgAaQBkAGUAVQBwAGQAYQB0AGUAcwAu
# AHAAcwAxoRaAFGh0dHA6Ly9taWNyb3NvZnQuY29tMA0GCSqGSIb3DQEBAQUABIIB
# AHZ9K4RnjmUcft3A4qz9ocWCtwwI+POlNfdLlMtRXNiozC29uFG9/wEoWHx2fxj/
# 8rYTz8wW0Q1i+QGJsoV/uoqEm/cg1ZrxnvbxUBKB65seVhwdjMkXD9GLrUFdbg+Q
# jU+1OnH33gATOFuTLUgKssesMqrgi3Z9j8Q8QpMBRyH8sQC/gl6oRTpTe5ygy+EN
# i4rP1qeEp4xL/73YLVa9skEC/Yzlvlpudti/hpL6rqUOeXxRkWVQCrVpKROnQgod
# BWtz3ynuEOx5AreBlOW9lwSjQ8YoEPpChkl7Tpt5hlAjqbh/m5tpbsXT7W80tR+r
# xAvgLMIMc+7r06QxLz+wCXahggIoMIICJAYJKoZIhvcNAQkGMYICFTCCAhECAQEw
...
# AxMYTWljcm9zb2Z0IFRpbWUtU3RhbXAgUENBAhMzAAAAcPQYvyMh/FCdAAAAAABw
# MAkGBSsOAwIaBQCgXTAYBgkqhkiG9w0BCQMxCwYJKoZIhvcNAQcBMBwGCSqGSIb3
# DQEJBTEPFw0xNTA2MTUyMDA2NTdaMCMGCSqGSIb3DQEJBDEWBBTB2JZDVz7JAiHL
# a88BU0q7pIrYXTANBgkqhkiG9w0BAQUFAASCAQCJBSaHAaRVjnQvcd0jwntLuxrF
# YsT4/X45+r5q18wM/nmbB7dAc5ac2WGXywXtsujjxH4Vx88El1COJLczwUe/HEhl
# bYqD6Wf1tavPGjLZdAIU7OWJEcBKN9OSG1mFhKaI3ne3/uxpPEiXwx+JfNng1Tn+
# EFoMWdYLKbIFKvPUh2aAl8YlN47AX8bBPUohOZW9q6FLMzi7yIzdldohPfWHn9p/
# uWPl4itMoKrigsItLrsEtrFlDleqRiOULwjjk5lLYOjNzAQ5cZ+MzhCTMgkTTU9O
# YED/rrWw/sDGo10R9jF4PJg+TS/BxOOGEWv4EyDP/3IHmoPcW34WD2OTf7A/
