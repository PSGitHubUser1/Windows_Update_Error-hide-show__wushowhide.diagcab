...
$temp = $result.Updates | select Title , IsHidden | where IsHidden -eq $false
...
if($Global:RootCauseDetected -eq $false)
{
	ForEach ($item in $Global:SelectedUpdate.Split(";"))
    {
		ForEach($i in $temp)
		{
			if($item -eq $i.Title)
			{
				Update-DiagRootCause -id "RC_UnHideUpdates" -iid $i.Title -Detected $Global:RootCauseDetected -Parameter @{"VF_Value" = $i.Title} 
...
else
{
	ForEach ($item in $Global:SelectedUpdate.Split(";"))
    {
		ForEach($i in $temp)
		{
			if($item -eq $i.Title)
			{
				Update-DiagRootCause -id "RC_UnHideUpdates" -iid $i.Title -Detected $true -Parameter @{"VF_Value" = $i.Title} 
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUDCFdOWVBQ/VV7P12XKbMI0ow
# uxagghWCMIIEwzCCA6ugAwIBAgITMwAAAHGzLoprgqofTgAAAAAAcTANBgkqhkiG
...
# HAYKKwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFHQ8
# /+P+R73y5/VdQyzJbTTj/SZvMGoGCisGAQQBgjcCAQwxXDBaoECAPgBXAHUAUwBo
# AG8AdwBIAGkAZABlAF8AVgBGAF8AVQBuAEgAaQBkAGUAVQBwAGQAYQB0AGUAcwAu
# AHAAcwAxoRaAFGh0dHA6Ly9taWNyb3NvZnQuY29tMA0GCSqGSIb3DQEBAQUABIIB
# ABF/dNZNDFPZxe86QOsP++UcOr555/LjePDXKDDqxRqIP5ggSJQPVXmG0lTvPGKL
# 6ZciQ3ErxMR8EjqAhwO2rWu5uW6wogCr1Gxh/gkkTq5yvdSIuPV8ffFGUpKjKV+z
# qj0b86HyUYIG1SR+A3DF07y/l8DiYatEkXoteZSpaknkgN7+4PjNK65xHMs9Lkj+
# 7T8c9o/E/pHyfEwe9eRL2cI9IPp/jwtJKKN8wNvAil446gY62c0ay7JwQr/S7HM4
# XPvJgDmaFWJBP30R6UnnElESrNnrwTYTLZTn6INnNRemXSLLXs41TklefMyY3PZL
# vmI5NL2tmfTacfGY9y85ftqhggIoMIICJAYJKoZIhvcNAQkGMYICFTCCAhECAQEw
...
# AxMYTWljcm9zb2Z0IFRpbWUtU3RhbXAgUENBAhMzAAAAcbMuimuCqh9OAAAAAABx
# MAkGBSsOAwIaBQCgXTAYBgkqhkiG9w0BCQMxCwYJKoZIhvcNAQcBMBwGCSqGSIb3
# DQEJBTEPFw0xNTA2MTUyMDA2NTdaMCMGCSqGSIb3DQEJBDEWBBR/gqlZ6NWQN3QS
# t2Ca2zd8ROZF4TANBgkqhkiG9w0BAQUFAASCAQCMGMri7nwtz23bTJWVuCcFGzRP
# O3y4Nd/10n8FuvkBybTGRaMaGuK+PeP8FOvOYx3GY/GAUTkW2V2QQiQzf8hqFTWI
# RpFBywqcpOofIBO+T+fbpY1z7FL1JbczmeP2vwpQ+986LzOwEFQBrf9S6+KAMaMh
# Q2IpCWt6N4w3xkQKSj7N3jcJervb9hQ6V+Hk14i/1Os6hFfaS7rkafwj/Lb3VphR
# lk3OYlhCOQGFp65xuxKPg0xYXiqxVf/y9xVw2o5DQ0RcGl+hUvebh+AaD5NRwKVc
# tinoFTKWdlpjZ8x6AxVLKQcf9UJe1299pUIRz3fasibPKOv/cAh0ifoEjjup
