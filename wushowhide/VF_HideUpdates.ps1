...
$temp = $result.Updates | select Title , IsHidden | where IsHidden -eq $true

if($Global:RootCauseDetected -eq $false)
{
	ForEach ($item in $Global:SelectedUpdate.Split(";"))
    {
		ForEach($i in $temp)
		{
			if($item -eq $i.Title)
			{
				Update-DiagRootCause -id "RC_HideUpdates" -iid $i.Title -Detected $Global:RootCauseDetected -Parameter @{"VF_Value" = $i.Title} 
...
else
{
	ForEach ($item in $Global:SelectedUpdate.Split(";"))
    {
		ForEach($i in $temp)
		{
			if($item -eq $i.Title)
			{
				Update-DiagRootCause -id "RC_HideUpdates" -iid $i.Title -Detected $true -Parameter @{"VF_Value" = $i.Title} 
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUK+kcK/tvOiFgwyPm6NxzsOgL
# ANSgghWCMIIEwzCCA6ugAwIBAgITMwAAAHGzLoprgqofTgAAAAAAcTANBgkqhkiG
...
# HAYKKwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFA6W
# SV8jzcH7bhzfqnkbcMsb+dggMGYGCisGAQQBgjcCAQwxWDBWoDyAOgBXAHUAUwBo
# AG8AdwBIAGkAZABlAF8AVgBGAF8ASABpAGQAZQBVAHAAZABhAHQAZQBzAC4AcABz
# ADGhFoAUaHR0cDovL21pY3Jvc29mdC5jb20wDQYJKoZIhvcNAQEBBQAEggEAV6s9
# GBzrcEux28kzeD2kdsjQPVK4ieBFq9tO05/gevyObCLCew4lIYL18wfcRg1vVRn6
# 0oATJEoVC9UdHKKjICap96Oj5KKr7gjaHAww6kr0kpJYGlvD6FeGi9KWJIebfwg8
# Ld+vDCEBcjolcAbkBjsVrCKzucIfAsQRFi13gJD+I76xXLfDPkaDuGLPLZ/jcaO1
# pukVPR/Ousf3u7vXTAwq8hlZ91XKNx8JFsF+3eKqKdb+/YRUz5n3UT9Nih6FALaE
# uQq1CwPv2p1Z10Q4+kSir66nvMdYtok9fWTYDJDqKhvgCZulv7Lv63lhn5cDauRh
# yvpv87BBLiiUOG61uqGCAigwggIkBgkqhkiG9w0BCQYxggIVMIICEQIBATCBjjB3
...
# MQ8XDTE1MDYxNTIwMDY1N1owIwYJKoZIhvcNAQkEMRYEFLWuTRcXEWYwjfObv02o
# LbTNeH3eMA0GCSqGSIb3DQEBBQUABIIBAEXB0zZSWxxEFRMCv0bbYq7hvoEELYWu
# /TBt45X2+UdZi4fo/pO9NqHJQNAfAB3Oi2Ud/AtfBQ3LJG+Mf1TwbDX1LRAt4F5b
# Nj5Dheb3cgT9DpujliLEMWgI+YBgKarQBEgyHuvdYAhTgDVfPrpRaUD6BEqk4hXr
# bolwZMUWVMLctPDoZNbuC7Dn80iy46NeHrUiFOe1SfBh3ea4PRPmn7UoqzRzYt82
# YygOUMzzaXako28E4o8MDPhsmsAyTdHJwLcHcOvM834RwjnT/gcxQpy6GLbU1n/G
# /suV3lmxPro2tDP4KEWAGpKVIgqQbBtrjlgHfBH3jlIxdyhLew0SAqM=
